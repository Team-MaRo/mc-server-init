...
- **Nix flake:** `nix build .#packages.x86_64-linux.default` (or
  `aarch64-linux`) → `result/bin/mc-server-init`. Crate dependencies:
  [`nix`](https://crates.io/crates/nix) (syscalls; re-exports `libc`) and
...
- **Releases:** [release-please](https://github.com/googleapis/release-please)
  cuts versioned GitHub Releases from conventional commits; CI attaches portable
  `mc-server-init-linux-{amd64,arm64}` binaries (system ELF interpreter, glibc
  ≥ 2.34) plus build-provenance attestations.
